...
const AnimatedWhiteTextSvg = ({
    ...
    useGSAP(() => {
        ...
        split.lines.forEach((line) => {
            ...
            wrapper.className = "relative w-fit inline-block"; // Removed isolate
            
            line.insertBefore(wrapper, allWords[0]);
            
            // Create overlay first so it's naturally behind in the DOM
            const overlay = document.createElement("div");
            overlay.className =
                "absolute left-0 top-1/2 h-[calc(50%+10px)] bg-gradient-to-r from-[#e0f4eb] via-[#b8e8e5] to-[#90dadc] opacity-60 rounded-sm pointer-events-none z-0"; // Set z-index directly in class
    ...
    return (
        <div 
            ref={textWrapperRef} 
            className="relative overflow-hidden w-full sm:w-[90%] md:w-[85%] lg:w-[80%] xl:w-[75%] mx-auto"
        >
            <h1
                ref={textRef}
                style={{ fontFamily: "'Afacad Flux', serif" }}
                className={`${className} whitespace-pre-wrap z-10`} // Added z-10 here
            >
                {text}
            </h1>
        </div>
